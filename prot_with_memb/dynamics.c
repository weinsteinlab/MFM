...

{
	...
	double pos_exp, neg_exp, Iv_integrand, Ip_integrand, Is_integrand, const_rand, dt, laplacian_term, sigma_ion, **order_parameter_final;
	double hx, hy, hz, origin_x, origin_y, origin_z, check_sum, average_op, average_sigma, surf_z, **dFdEta, up, down, sum_op;
...
/****** READ PARAMETER FILE ***/
	...
	const_rand = 0.0;
	noise = 0.0;
	average_op = 2.0*fabs(area_per_lipid*average_sigma/valency)-1; /* this is average fraction of chargd lipids */
...
/*** READ APBS MAPS ****/
	...
	order_parameter = (double **) malloc(nx * sizeof(double *));
        ...
        order_parameter_final = (double **) malloc(nx * sizeof(double *));
        order_parameter_final[0] = malloc(nx * ny * sizeof(double));
        for(i = 1; i < nx; ++i)
                order_parameter_final[i] = order_parameter_final[0] + i * ny;
                ...
                for (j=0;j<ny;++j) {
                        for (k=0;k<nz;++k) {
                                if ((origin_z+hz*k) == surf_z) {
                                        ...
                                        order_parameter[i][j] = 2.0*fabs(dataCharge[i*ny*nz+j*nz+k]*area_per_lipid/valency)-1;
                                }
                        }
                }
        }
...
//			niu2[i][j] = GRandom(0,1);
			up = (1+order_parameter[i][j])*(1-average_op);
			down = (1+average_op)*(1-order_parameter[i][j]);
			dFdEta[i][j] = 0.5*(log(up/down) - valency*SurfPotential[i][j]);  /* calculating gradients of F with respect to eta */
		}
	}
...
//			noise = niu1[nn_x_right[i][j]][j] - niu1[i][j] + niu2[i][nn_y_down[i][j]]- niu2[i][j];
			laplacian_term = dFdEta[nn_x_left[i][j]][j] + dFdEta[nn_x_right[i][j]][j] + dFdEta[i][nn_y_up[i][j]] + dFdEta[i][nn_y_down[i][j]] - 4*dFdEta[i][j];
        ...
        for (i=0;i<nx;++i) {
                for (j=0;j<ny;++j) {
                        for (k=0;k<nz;++k) {
                                if ((origin_z+hz*k) == surf_z) {
					if (average_sigma < 0) 
						dataCharge[i*ny*nz+j*nz+k] = -(order_parameter[i][j]+1)*0.5*valency/area_per_lipid;
					else
						dataCharge[i*ny*nz+j*nz+k] = (order_parameter[i][j]+1)*0.5*valency/area_per_lipid;
                                }
                        }
