...
#define a_lip 70.0
#define valency 4.0
#define f_memb f_memb_totalenergy 
#define f_syn f_syn_totalenergy
...

{
	...
	for (i=0; i<nx; ++i) {
	        ...
	        fscanf (in1ptr, "%lf\n", &el_ene[i]);
		el_ene[i] = el_ene[i] / (R*T/1000);
		printf ("%d  %lf  %lf  %lf  %lf\n", i, mixing[i], el_ene[i], fourth_term[i], el_ene[i]+mixing[i]+fourth_term[i]-(f_memb/(R*T/1000)+f_syn/(R*T/1000)+fourth_term[0]));
	}
}	
